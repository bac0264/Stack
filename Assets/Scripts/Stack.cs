...
namespace OB.Game
{
    ...
    public class Stack : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            transform.DOMoveX(-10, _blockConfig.BlockPingPongLoopTime).SetLoops(-1, LoopType.Yoyo).SetEase(Ease.Linear);
        }
        ...
        public void PlaceBlock()
        {
            ...
            if (BlockUtils.Overlap(ref movingBlockBounds, ref topBlockBounds))
            {
                ...
                if (BlockUtils.PerfectOverlap(ref movingBlockBounds, ref topBlockBounds, _diffConfig.PerfectPlacementErrorMargin))
                {
                    ...
                    {
                        _topBlock = _blockFactory.GenerateStaticBlockFadeHalo(ref newTopBounds, staticObjectParent);
                        NextBlock();
                    }
                    else if (_perfectPlacementCount < _perfectPlacementsLevel1 + _perfectPlacementsLevel2) // level 2
                    {
                        _topBlock = _blockFactory.GenerateStaticBlockPulseHalo(ref newTopBounds, _perfectPlacementCount - _perfectPlacementsLevel1 + 1, staticObjectParent);
                        NextBlock();
                    }
                    else // level 3
                    {
                        if (_topBlock.Bounds.size == _blockConfig.BlockStartingSize)
                        {
                            ...
                            _topBlock = _blockFactory.GenerateStaticBlockReward(ref newTopBounds, _perfectPlacementsLevel2, staticObjectParent);
                            NextBlock();
                        }
                        else
                        {
                ...
                else
                {
                    ...
                    _topBlock = _blockFactory.GenerateStaticBlock(ref overlapBounds, staticObjectParent);
                    NextBlock();
                }
            ...
            {
                DOTween.Pause(staticObjectParent.gameObject);
        ...
        void NextBlock()
        {
            ...
            // spawnOffset + movingBounds.center,
            // movingBounds.center - spawnOffset
            _movingBlock = _blockFactory.GeneratePingPongBlock(ref movingBounds, new Vector3(distance,movingBounds.center.y,movingBounds.center.z), new Vector3(distance, movingBounds.center.y, movingBounds.center.z), transform, true);
        }
        ...
        void GenerateInitialStack()
        {
            ...
            {
                _topBlock = _blockFactory.GenerateStaticBlock(ref blockBounds, staticObjectParent);
